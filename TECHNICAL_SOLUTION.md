...
### A. 文件导入链路重构（File Conversion）

为解决“导入本地媒体时主线程短暂卡住”的问题，导入流程从同步重处理改为两阶段模型：

1. 先在主线程快速创建占位文件项（`importStatus = .loading`），立即渲染列表
2. 在后台队列执行时长读取、音频抽取、重采样与波形采样
3. 回到主线程写回 `duration / waveformSamples` 并标记 `importStatus = .ready`

关键点：

- UI 感知从“阻塞等待”变为“即时入列 + 渐进完成”
- 导入弹窗使用不确定进度，避免误导性百分比
- 支持导入取消：取消任务并清理仍在 loading 的占位项

### B. 批量降噪可中断机制

批量处理新增“可中断”能力，贯穿 ViewModel 与底层处理器：

- `AudioViewModel`
  - 新增停止意图状态与 `stopProcessing()`
  - 中断当前活跃 `FFmpegDenoiser`
  - 阻止队列继续处理后续文件
- `FFmpegDenoiser`
  - 新增 `ProcessingCancellationToken`
  - 在耗时循环和关键阶段插入取消检查点
  - 取消时抛出统一错误 `cancelled`

行为约束：

- 取消不计为业务失败，当前项状态回退到可重试态（`idle`）
- 处理中止后不会继续推进剩余文件

### C. AVAsset 异步加载稳定性增强

针对 `mp4/mov` 读取 `duration` / `tracks(audio)` 的偶发超时，做了同步桥接层治理：

- 调整超时参数与执行上下文，避免主线程阻塞导致的超时放大
- 收敛并发写入风险，降低超时后的崩溃概率
- 增加调试日志用于区分：
  - duration 超时
  - tracks(audio) 超时
...
- `L10nKey` 增加稳定 key
- `Localizable.xcstrings` 完整补齐多语言翻译（`zh-Hans/zh-Hant/en/ja`）
- 视图层避免硬编码用户可见字符串
