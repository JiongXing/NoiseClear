...
final class FFmpegDenoiser: Sendable {
    ...
    private func denoiseVideo(
        ...
        // 视频直通写入
        group.enter()
        let videoQueue = DispatchQueue(label: "com.noiseclear.denoise.video", qos: .userInitiated)
        ...
        // 降噪音频写入
        group.enter()
        let audioQueue = DispatchQueue(label: "com.noiseclear.denoise.audio", qos: .userInitiated)
